...
class IndexController extends Controller
{
   public function index()
   {
    ...
    $TotalJob= PostJob::count();
        ...
        $SuccessStories =DB::table('success_stories')
        ->join('users','users.id','=','success_stories.user_id')
        ->join('employer_profiles','employer_profiles.id','=','success_stories.emp_id')
       ->orderBy('success_stories.created_at', 'DEC')->get();

      // dd($SuccessStories);  

      $SuccessStoriescan =DB::table('success_stories')
      ->join('users','users.id','=','success_stories.user_id')
     ...
     array_push($alljobCan,$SuccessStories, $SuccessStoriescan);
   ...
   // dd( $alljobCan); 
        ...
        return view('Layout.index',compact('alljobCan','Success','TotalJob','TotalCandidate','TotalVideoCvs','TotalAnsweredQuestions','RecentlyAddedJobs','SuccessStories','TopCandidate'));
   }
   ...
   {
   $data= PostJob::search($words)->orderBy('job_for')->paginate(8);
...
$count=count($jobs);
...

        {
               ...
               $jobs=PostJob::search($words)->orderBy('job_for')->paginate(8);
